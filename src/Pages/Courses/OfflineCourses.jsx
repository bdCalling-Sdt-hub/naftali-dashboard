...
const OfflineCourses = () => {
  ...
  const activeCoursesCount = courseData.filter(
    (course) => course.status === "active" && course.type === "platform"
  ).length;
  const inactiveCoursesCount = courseData.filter(
    (course) => course.status === "completed" && course.type === "platform"
  ).length;
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold my-5">Offline Courses</h1>
      <Tabs
